...
def make_potrace_svg_fragment(bitmap_uint8, work_w, work_h):
...
async def stencil(
    ...
    # ۱. خط اصلی طرح (همان روش مرحله‌ی Edge Detection)
    blurred = cv2.GaussianBlur(gray_work, (3, 3), 0)
    edges = cv2.Canny(blurred, low_threshold, high_threshold)
    bitmap = cv2.bitwise_not(edges)
    try:
        lineart_svg_fragment = make_potrace_svg_fragment(bitmap, work_w, work_h)
    except Exception as e:
        return JSONResponse(
            status_code=500,
