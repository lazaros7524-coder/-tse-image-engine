...
<body>
  ...
  <div class="box">
    ...
    <br><br>
    <label>شدت انحنای هاشور: <span id="curveVal">8</span></label><br>
    <input type="range" id="curveRange" min="0" max="60" value="8"><br>
    <label>فاصله‌ی ردیف‌های هاشور: <span id="spacingVal">5</span></label><br>
    <input type="range" id="spacingRange" min="2" max="15" value="5"><br><br>
    <button id="shadeBtn">تولید هاشور سایه‌روشن</button>
    <button id="stencilBtn">تولید استنسیل کامل</button>
  </div>
...
curveRange.addEventListener('input', () => curveVal.textContent = curveRange.value);
spacingRange.addEventListener('input', () => spacingVal.textContent = spacingRange.value);
...
shadeBtn.addEventListener('click', async () => {
  ...
  try {
    const shadeParams = new URLSearchParams({
      curve_strength: curveRange.value,
      row_spacing: spacingRange.value
    });
    const response = await fetch(`https://tse-image-engine.onrender.com/shade?${shadeParams.toString()}`, {
      method: 'POST',
      body: formData
    });
...
stencilBtn.addEventListener('click', async () => {
  const file = fileInput.files[0];
  if (!file) {
    statusEl.textContent = 'لطفاً اول یک عکس انتخاب کنید.';
    return;
  ...
  statusEl.textContent = 'در حال تولید استنسیل کامل (خط + هاشور)...';
  resultEl.innerHTML = '';

  const formData = new FormData();
  formData.append('file', file);

  const stencilParams = new URLSearchParams({
    low_threshold: lowRange.value,
    high_threshold: highRange.value,
    curve_strength: curveRange.value,
    row_spacing: spacingRange.value
  ...
  try {
    const response = await fetch(`https://tse-image-engine.onrender.com/stencil?${stencilParams.toString()}`, {
      method: 'POST',
      body: formData
    });
    ...
    const svgText = await response.text();
    resultEl.innerHTML = svgText;
    statusEl.textContent = '✅ استنسیل کامل با موفقیت تولید شد.';
  } catch (err) {
    statusEl.textContent = 'خطا در اتصال به سرویس: ' + err.message;
  }
